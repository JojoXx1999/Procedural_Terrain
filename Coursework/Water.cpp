...
Water::Water(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int lresolution) :
	PlaneMesh(device, deviceContext, lresolution) //inherit from plane mesh
{
	resolution = 149;
	srand(time(NULL));
	vertexBuffer = NULL;
	heightMap = new float[resolution * resolution];
	BuildHeightMap(-100, 0);
...
void Water::Move(float adjust) {
	//Allow the user to control the height of the water plane
	for (int j = 0; j < (resolution); j++) {
		for (int i = 0; i < (resolution); i++) {
			heightMap[(j * resolution) + i] = (heightMap[(j * resolution) + i] * adjust);
		}
	}
}

void Water::Update(ID3D11Device* device, ID3D11DeviceContext* deviceContext) {
	Regenerate(device, deviceContext, startH, startF);
}

void Water::BuildHeightMap(float h, float f) {
	float position;
	for (int j = 0; j < (resolution); j++) {
		for (int i = 0; i < (resolution); i++) {
...
//
void Water::Regenerate(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float h, float f) {
