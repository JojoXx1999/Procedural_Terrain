...
void LightShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, ID3D11ShaderResourceView* texture3, ID3D11ShaderResourceView* texture4, ID3D11ShaderResourceView* texture5, Light* light, Light* point, float bRate, float height, float lowest, float startWater, float directional, float fogS, float fogE, float fogStrength, XMFLOAT4 bg)
{
	...
	lightPtr->ambient[1] = point->getAmbientColour();
	lightPtr->diffuse[1] = point->getDiffuseColour();
	lightPtr->position[0] = XMFLOAT4(point->getPosition().x, point->getPosition().y, point->getPosition().z, 0);

	lightPtr->padding = 0.f;
	lightPtr->bg = bg;
	lightPtr->directional = directional;
	lightPtr->height = height;
	lightPtr->lowest = lowest;
	lightPtr->startWater = startWater;
	lightPtr->fogS = fogS;
	lightPtr->fogE = fogE;
	lightPtr->fogStrength = fogStrength;
	lightPtr->blend = bRate;
